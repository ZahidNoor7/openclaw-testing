...
export function getOrCreateTenantApiKey(orgId: string): string {
  ...
  db.prepare("INSERT INTO tenant_api_keys (id, org_id, created_at) VALUES (?, ?, ?)").run(
    key,
    orgId,
...
export function regenerateTenantApiKey(orgId: string): string {
  ...
  const key = randomBytes(32).toString("hex");
  db.prepare(
    "INSERT OR REPLACE INTO tenant_api_keys (id, org_id, created_at) VALUES (?, ?, ?)",
  ).run(key, orgId, new Date().toISOString());
  return key;
}

export function validateTenantApiKey(apiKey: string): { orgId: string } | null {
  const db = getAuthDb();
  const row = db.prepare("SELECT org_id FROM tenant_api_keys WHERE id = ?").get(apiKey) as
    | Record<string, string>
    | undefined;
  if (!row) {
    return null;
